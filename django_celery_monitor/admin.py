...
def colored_state(task):
    ...
    return format_html('<b><span style="color: {0};">{1}</span></b>'.format(color, state))
...
def eta(task):
    ...
    return format_html(escape(make_aware(task.eta)))
...
def tstamp(task):
    ...
    return format_html('<div title="{0}">{1}</div>'.format(
...
def name(task):
    ...
    return format_html('<div title="{0}"><b>{1}</b></div>'.format(
...
class TaskMonitor(ModelMonitor):
    ...
    list_display = (
        ...
        colored_state,
        name,
        fixedwidth('args', pretty=False),
        fixedwidth('kwargs', pretty=False),
        eta,
        tstamp,
        'worker',
    )
